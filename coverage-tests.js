...
test('check region by element within shadow dom', {
  ...
  test({driver, eyes}) {
    ...
    const shadowRoot = shadowRootHost.getShadowRoot()
    const nestedShadowRootHost = shadowRoot.findElement('#has-shadow-root-nested > div')
    const nestedShadowRoot = nestedShadowRootHost.getShadowRoot()
    const element1 = shadowRoot.findElement('h1')
    const element2 = nestedShadowRoot.findElement('div')
